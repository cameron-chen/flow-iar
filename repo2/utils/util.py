...
from .torch_layers import (CnnExtractor, CombinedDummyExtractor,
                           CombinedExtractorExtend, GnnExtractor)
...
def args_parser(args):
    ...
    elif getattr(args, "features_extractor_class", None) == "combineddummy":
        policy_kwargs['features_extractor_class'] = CombinedDummyExtractor
        policy_kwargs['features_extractor_kwargs'] = dict()
        if policy_kwargs.get('net_arch', None) is None:
            policy_kwargs['net_arch'] = [dict(pi=[64], vf=[64])] # default net_arch if feature extractor exists, follow SB3
        else:
    ...
    if args.normalize: 
        normalize_kwargs = dict( 
            ...
            norm_obs_keys = args.norm_obs_keys,
        )
